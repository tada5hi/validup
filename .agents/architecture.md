...
> **The issue model lives in [`@ebec/core`](https://github.com/tada5hi/ebec)**, a zero-dependency package with no `engines` floor — the same one `ValidupError` takes `BaseError` from. It is **not** re-exported by `packages/validup/src/index.ts`; every module here imports it directly. Everything in this section down to `ValidupError` describes types and functions defined *there* — file paths below are the `ebec` repo's `packages/core/src/issue/`, not this repo's. A change to any of it belongs in that repo. See [structure.md](structure.md#the-issue-model-lives-in-ebeccore) for why it was extracted and what moving it broke for consumers.
>
...
`@ebec/core` (`packages/core/src/issue/types.ts` upstream) — `Issue = IssueItem | IssueGroup` (discriminated by `type`).
...
- **`defineIssueItem` and `createValidupError` enforce the per-code `data` contract at compile time** via conditional-type signatures (`DefineIssueItemData<C>` in `@ebec/core`'s `issue/define.ts` and `CreateValidupErrorTail<C>` in this repo's `src/helpers/create-error.ts`; the shared `ResolveIssueCode<C>` helper in `@ebec/core`'s `issue/types.ts` handles the `code: undefined → VALUE_INVALID` default). `@validup/validator-js`'s `test:types` covers this **across the package seam** — it is the only automated check in this repo that the gatekeep still enforces after the extraction, so keep it running. Passing `IssueCode.MIN_LENGTH` without `data: { min }` is a compile error; passing `IssueCode.STRONG_PASSWORD` with `data: { pointsPerUnique: 5 }` is a compile error (scoring weight, not a documented requirement key); passing `IssueCode.EMAIL` with any `data` is a compile error.
...
- **Augment `'@ebec/core'`, never `'validup'`.** `declare module '@ebec/core' { interface IssueDataByCode { email_taken: { existingUserId: string } } }` merges onto the real interface, and `ParameterizedIssueCode` — computed in that module — picks the new code up. Targeting `'validup'` used to work through the star re-export; with the re-export gone it now declares a fresh, unrelated `IssueDataByCode` inside validup's own module scope. That **compiles silently**: no error marks the augmentation as inert, `ParameterizedIssueCode` never sees the code, and `defineIssueItem` / `createValidupError` fall through to the ad-hoc-string branch where `data` is optional and untyped. A stale `'validup'` augmentation is therefore a gatekeep that looks present and does nothing — see [testing.md](testing.md#the-issue-model-is-tested-in-ebec-not-here).
...
- `isValidupError(e)` checks `instanceof ValidupError`, then the error's `@instanceof` marker chain (`VALIDUP_ERROR_INSTANCE`, via `matchesInstanceof` — covers a duplicate copy of the package and an error rehydrated from `toJSON()` output, since the marker's serialized description string survives a JSON round trip even though the symbol itself doesn't), and only falls back to duck-typing when neither matches: a non-empty `issues` array whose members all pass `isIssue`, or an empty one on an error whose `code` is `'VALIDUP_ERROR'` — `code` survives serialization, so it catches a zero-issue `ValidupError` from before the marker existed (validup <= 2.0.0) that `instanceof`/the marker can no longer see. An empty `issues` array with no `VALIDUP_ERROR` code is not sufficient on its own: every `@ebec/core` `BaseError` carries an `issues` array by default. Use `isValidupError` across package boundaries — direct `instanceof ValidupError` may miss errors from a duplicate copy of the package. Same duck-typed-fallback pattern (without a marker) for `isRunSyncViolation` (private, internal to `container/`) and `isPathsStrictViolation` (exported — consumers catch `PathsStrictViolationError` to read the stale `pathsToInclude` / `pathsToExclude`).
