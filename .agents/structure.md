...
The path codec is currently **not** `pathtrace`'s `arrayToPath` / `pathToArray` / `getPathValue` / `setPathValue`, even though the core already depends on them. **This is a temporary fork, not a design goal — it should be deleted once the upstream blockers clear.** Maintaining the same traversal logic in two repos is the thing to avoid; the local copy exists only because using pathtrace today would be incorrect.

Blocked on [tada5hi/pathtrace#200](https://github.com/tada5hi/pathtrace/issues/200):

1. **`setPathValue` picks array-vs-object auto-creation from the *current* key rather than the *next* one**, so `a[0].b` materializes as `{ a: { '0': [] } }` and the value is lost at any serialization boundary. (The core hits this too — see the note on `Container.finalizeOutput` below.)
2. **`setPathValue` refuses to replace a pre-existing `null` / primitive intermediate**, silently dropping the write. `{ address: null }` is ordinary form state.
3. **`arrayToPath` emits bracket notation** (`['tags', 0]` → `'tags[0]'`). This module's canonical form is **pure-dotted**, so that `pathKey(issue.path)` and `pathFromKey(fieldKey).join('.')` are directly comparable; bracket form breaks ancestor-prefix matching, since `'tags[0].name'.startsWith('tags.')` is false. `pathToArray` additionally keeps non-numeric brackets, so `fields.at('meta[locale]')` would start resolving `state.meta['[locale]']`.

Items 1 and 2 are outright bugs; item 3 needs a dotted-output option upstream. **When #200 lands: delete `readNested` / `writeNested` in favour of `getPathValue` / `setPathValue` first (those need only the bug fixes), and retire `pathKey` / `pathFromKey` only if the codec option ships.** Adopting pathtrace here also means adding it to `packages/vue/package.json` as a direct dependency — the vue package has none today, and relying on root hoisting is the bug fixed in 42b3de1 for zod / validator-js.

> ⚠️ Item 1 affects the **core**, not just vue: `Container.finalizeOutput` expands its flat dotted output with `setPathValue`, so `mount('items.*.name', …)` run with the default `flat: false` returns `{ items: { '0': [] } }` and loses every validated array value on serialization. The existing glob specs miss it because they all pass `{ flat: true }` — the branch that skips `setPathValue` — and use object keys rather than array indices.
