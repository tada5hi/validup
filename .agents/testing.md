...
**The catch-all entry is pinned as `any`, exactly** (`_t455f` / `_t455g`), and that is not redundant with the widening cases even though `any` is what makes them pass. Mutation-verified: giving `FieldEntry` an index entry of `FieldState<T[K]> | ((path: string) => FieldState<any>)` satisfies the #423 leg — the union admits the `at` method, so all four widening assertions stay green — while handing consumers a union they must narrow before touching `$model`. Only the exact-type cases and the `.$model.value` cases catch it. **When a type is widened to `any` to make an assignability constraint pass, assert the `any`: other types satisfy that constraint too, and they are worse.**

The suite also carries one runtime `it()` against an index-signature entity, exercising a declared key, an **undeclared** one (`extra`, present only via the index signature — the key the `any` case describes), and one absent from state entirely. It is not decorative: the missing `| undefined` on typed keys is only honest because the Proxy really does materialise a `FieldState` for every key.
