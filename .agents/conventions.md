...
- **Index barrels**: every directory under `src/` has an `index.ts` re-exporting its members. Keep this pattern when adding modules — `src/index.ts` re-exports each subdir wholesale. Four modules are deliberately excluded because they are internal plumbing, not API: `validup`'s `container/run-sync-violation.ts` and `container/structural-throw.ts`, plus `@validup/vue`'s `helpers/collector.ts` and `helpers/projection.ts`. Don't "fix" them by adding a barrel line — `src/index.ts` re-exports `./helpers` wholesale, so a line there is a semver-protected public export. Specs reach these modules by direct path (`import { … } from '../../src/helpers/projection'`).
