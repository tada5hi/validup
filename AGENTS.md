...
The repo is an **Nx-managed npm workspace monorepo** containing the core library, four integration packages (`@validup/standard-schema`, `@validup/zod`, `@validup/express-validator`, `@validup/vue`), a private `docs/` workspace that builds the VitePress site published to GitHub Pages, and a private `playground/` tree for runnable demo apps (currently `playground/vite-vue`, a Vite + Vue 3 multi-route showcase of `@validup/vue`). The project is pre-1.0 and explicitly marked as work-in-progress. All published packages are licensed Apache-2.0.
